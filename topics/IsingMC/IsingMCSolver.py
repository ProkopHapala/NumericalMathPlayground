# run it like this:
#   python -u -m IsingMCSolver | tee OUT
...
class IsingMCSolver(OpenCLBase):
    """Convenient wrapper around the brute-force and MC kernels in `cl/ising_mc.cl`.
    ...
    def solve_local_updates(self, W_sparse=None, Esite=None, Tsite=None, nTips=None, nSite=None, nMaxNeigh=None, params=default_params, bRealloc=True, initMode=3, bNoCoupling=False ):
        ...
        if nSite > 64: raise ValueError("[IsingMCSolver] nSite > 64 exceeds ulong bitmask limit.")
    ...
    def solve(
        ...
        if nSingle > 32: raise ValueError("[IsingMCSolver] nSingle > 32 exceeds kernel limit")

        # (Re)allocate GPU memory & upload inputs --------------------------------
        if bRealloc: self.realloc_buffers(nSingle, nTips)

        # print("IsingMCSolver::solve() dir(self):")
    ...
    def eval_oriented_hopping(self, posE: np.ndarray, rots: np.ndarray, orbs: np.ndarray, pTips: np.ndarray, bRealloc: bool = True):
        ...
        nTips   = pTips.shape[0]
        total   = nSingle * nTips
        print("IsingMCSolver::eval_oriented_hopping() nSingle: ", nSingle, " nTips: ", nTips, " total: ", total)
