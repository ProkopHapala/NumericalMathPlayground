"""
hydrogen_chain_1d.py — Comprehensive test harness for Order-N methods on a 1D hydrogen dimer chain.
Compares linear-scaling density estimates (FOE, GF, OMM) against exact O(N^3)
diagonalization. Supports configurable tight-binding parameters, overlap, and
multiple solver options with detailed diagnostics and plotting.
"""
import argparse
import sys
...
# add sibling directories to sys.path for cross-topic imports
_topic_root = Path(__file__).resolve().parent.parent
for _sub in ("DensityMatrix", "OMM"):
    _p = str(_topic_root / _sub)
