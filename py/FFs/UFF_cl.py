...
# ---- Force-field eval function for finite-difference Hessian ----
...
def make_ff_eval_fn(mol, ff='uff', do_nonbond=False):
    """Unified factory. ff in ('uff',). Returns (eval_fn, pos0, natoms, perm, enames)."""
    if ff == 'uff':
        return make_uff_eval_fn(mol, do_nonbond=do_nonbond)
    raise ValueError(f"Unknown ff={ff!r}, expected 'uff'")
